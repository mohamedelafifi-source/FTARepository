            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                HStack(spacing: 8) {
                    Button {
                        ...
                        Label("Family Tree", systemImage: "person.3")
                    }
                    ...
                    Button {
                        ...
                        Label("Paste/Parse", systemImage: "doc.on.clipboard")
                    }
                    .buttonStyle(.bordered)
    ...
    var body: some View {
        NavigationStack {
            contentHost
                    ...
                    ToolbarItemGroup(placement: .navigationBarLeading) {
                        ...
                        Menu("Main") {
                            ...
                            if let url = globals.selectedFolderURL {
                                ...
                                Text("Folder: none").foregroundColor(.secondary)
                            }
                            Divider()
                            ...
                            // FILE HANDLING
                            //=====================
                            Text("File Handling").font(.caption)
                ...
                .sheet(isPresented: $showUIKitPickerForLoad) {
                    DocumentPicker(contentTypes: [.json], allowsMultipleSelection: false) { urls in
                        showUIKitPickerForLoad = false
                        guard let url = urls.first else { return }
                        guard isLikelyJSON(url) else {
                            alertMessage = "Please select a .json file."
                            showAlert = true
                            return
                        }
                        do {
                            let imported = try loadMembers(from: url)
                            dataManager.membersDictionary.removeAll()
                            for m in imported { dataManager.membersDictionary[m.name] = m }
                            dataManager.linkFamilyRelations()
                            dataManager.assignLevels()
                            successMessage = "Loaded \(imported.count) members from \(url.lastPathComponent)"
                            showSuccess = true
                        } catch {
                            alertMessage = "Failed to load JSON: \(error.localizedDescription)"
                            showAlert = true
                        }
                    } onCancel: {
                        showUIKitPickerForLoad = false
                ...
                .sheet(isPresented: $showUIKitPickerForAppend) {
                    DocumentPicker(contentTypes: [.json], allowsMultipleSelection: false) { urls in
                        showUIKitPickerForAppend = false
                        guard let url = urls.first else { return }
                        guard isLikelyJSON(url) else {
                            alertMessage = "Please select a .json file."
                            showAlert = true
                        ...
                        do {
                            ...
                            for member in imported {
                                if var existing = dataManager.membersDictionary[member.name] {
                                    let parents = member.parents.filter { !existing.parents.contains($0) }
                                    let spouses = member.spouses.filter { !existing.spouses.contains($0) }
                                    let children = member.children.filter { !existing.children.contains($0) }
                                    let siblings = member.siblings.filter { !existing.siblings.contains($0) }
                                    if !parents.isEmpty || !spouses.isEmpty || !children.isEmpty || !siblings.isEmpty {
                                        existing.parents.append(contentsOf: parents)
                                        existing.spouses.append(contentsOf: spouses)
                                        existing.children.append(contentsOf: children)
                                        existing.siblings.append(contentsOf: siblings)
                                        dataManager.membersDictionary[member.name] = existing
                                        appended += 1
                                    }
                                } else {
                                    dataManager.membersDictionary[member.name] = member
                                    appended += 1
                                }
                            }
                            dataManager.linkFamilyRelations()
                            dataManager.assignLevels()
                            successMessage = "Appended/merged \(appended) members from \(url.lastPathComponent)"
                            showSuccess = true
                        } catch {
                            alertMessage = "Failed to append JSON: \(error.localizedDescription)"
                            showAlert = true
                        }
                    } onCancel: {
                        showUIKitPickerForAppend = false
